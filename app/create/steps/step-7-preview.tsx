...
/* ─── Publishing overlay with animated phases ─── */
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: `radial-gradient(ellipse 60% 40% at 50% 50%, rgba(236,72,153,0.15) 0%, transparent 70%)`,
        }}
      />
      ...
      <div className="relative z-10 flex flex-col items-center gap-8 px-6">
        ...
        >
          <div
            ...
            style={{
              background: `conic-gradient(from 0deg, ${PINK}, ${PURPLE}, ${PINK})`,
              padding: "3px",
            }}
          >
            <div
    ...
    <div className="relative mx-auto" style={{ width: "320px" }}>
      <div
        ...
        style={{
          ...
          boxShadow: `0 0 0 1px rgba(255,255,255,0.06), 0 0 60px rgba(236,72,153,0.15), 0 0 120px rgba(168,85,247,0.1), 0 40px 80px rgba(0,0,0,0.6)`,
        }}
      >
    ...
    <div style={{ background: "#070707", minHeight: "620px" }}>
      ...
      <div className="relative h-72 overflow-hidden">
        {coverPhoto ? (
          <>
            <div
              ...
              style={{ backgroundImage: `url(${coverPhoto})`, backgroundSize: "cover", backgroundPosition: "center", filter: "blur(16px) brightness(0.4)" }}
            />
          ...
          >
            <svg width="48" height="48" viewBox="0 0 24 24" fill={PINK} opacity={0.5} aria-hidden="true">
              <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
            </svg>
          </div>
        )}
        ...
        <div className="absolute bottom-4 left-4 z-30">
          <p className="font-serif text-2xl font-bold text-white" style={{ textShadow: "0 2px 12px rgba(0,0,0,0.8)" }}>
            {data.personName || "Sua pessoa"}
          </p>
        ...
        <div className="absolute top-10 right-4 z-30">
          <motion.div animate={{ scale: [1, 1.15, 1] }} transition={{ duration: 2, repeat: Infinity }}>
            <svg width="22" height="22" viewBox="0 0 24 24" fill={PINK} aria-hidden="true">
              <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
            </svg>
          </motion.div>
        </div>
      ...
      <div className="px-4 pb-8 space-y-4">
            ...
            <div className="grid grid-cols-4 gap-2">
              {[{ v: elapsed.days, l: "dias" }, { v: elapsed.hours, l: "horas" }, { v: elapsed.minutes, l: "min" }, { v: elapsed.seconds, l: "seg" }].map((item) => (
                <div key={item.l} className="text-center">
                  <motion.p key={item.v} initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} className="font-bold text-base" style={{ color: PINK }}>
                    {item.l === "dias" ? item.v : pad(item.v)}
                  </motion.p>
        ...
        {data.song && (
          <div className="rounded-2xl p-3 flex items-center gap-3" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}>
            <div className="w-10 h-10 rounded-xl flex-shrink-0 flex items-center justify-center" style={{ background: `linear-gradient(135deg, ${PINK}, ${PURPLE})` }}>
            ...
            <button onClick={() => setPlaying((p) => !p)} className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 cursor-pointer"
              style={{ background: `linear-gradient(135deg, ${PINK}, ${PURPLE})` }} aria-label={playing ? "Pausar" : "Reproduzir"}>
              {playing ? (
                <svg width="8" height="8" viewBox="0 0 12 12" fill="white" aria-hidden="true"><rect x="2" y="1" width="3" height="10" rx="1" /><rect x="7" y="1" width="3" height="10" rx="1" /></svg>
              ) : (
                <svg width="8" height="8" viewBox="0 0 12 12" fill="white" aria-hidden="true"><path d="M3 1.5L10.5 6 3 10.5V1.5z" /></svg>
              )}
            </button>
            {playing && (
              <div className="flex items-end gap-0.5 h-4 flex-shrink-0">
                {[3, 6, 4, 8, 5].map((h, i) => (
                  <motion.div key={i} className="w-0.5 rounded-sm" style={{ background: i % 2 === 0 ? PINK : PURPLE }}
        ...
        {data.message && (
          <div className="rounded-2xl p-4" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            <p className="text-sm text-foreground leading-relaxed italic font-serif">
              &ldquo;{data.message.slice(0, 140)}{data.message.length > 140 ? "..." : ""}&rdquo;
            </p>
          </div>
        )}
        ...
        {/* InfinityLove branding */}
        <div className="text-center pt-2">
          <p className="text-xs text-muted-foreground">Feito com amor em</p>
          <p className="text-xs font-bold mt-0.5" style={{ background: `linear-gradient(90deg, ${PINK}, ${PURPLE})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
            InfinityLove
          </p>
        </div>
      </div>
...
function SuccessState({ slug, personName }: { slug: string; personName: string }) {
  ...
  const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(
    `💕 ${personName}, fiz algo especial pra você!\n\nAbre esse link:\n${pageUrl}`
  )}`
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2 rounded-xl px-3 py-2.5" style={{ background: "rgba(255,255,255,0.06)" }}>
          ...
          <button
            onClick={handleCopy}
            className="text-xs font-semibold cursor-pointer flex-shrink-0 px-2 py-1 rounded-lg transition-colors"
            style={{
              color: copied ? "#fff" : PINK,
              background: copied ? `linear-gradient(135deg, ${PINK}, ${PURPLE})` : "transparent",
            }}
          >
        ...
        <p className="text-xs text-muted-foreground mt-3">
          Envie esse link para {personName || "ela"} pelo WhatsApp, Instagram ou onde quiser.
        </p>
      </motion.div>
      ...
      <motion.a
        ...
        whileHover={{ scale: 1.03, boxShadow: `0 0 50px rgba(37,211,102,0.4)` }}
        whileTap={{ scale: 0.97 }}
      ...
      <motion.button
        ...
        whileHover={{ scale: 1.03, boxShadow: `0 0 50px rgba(236,72,153,0.5)` }}
        whileTap={{ scale: 0.97 }}
...
export default function Step7Preview({ onBack }: StepProps) {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      if (data.relationshipStart) {
        fd.append("relationshipStart", data.relationshipStart)
      }
      if (data.message) {
  ...
  return (
    ...
    >
      {/* Publishing overlay */}
      <AnimatePresence>
        {(phase === "uploading" || phase === "saving" || phase === "generating") && (
          <PublishingOverlay phase={phase} />
        )}
      </AnimatePresence>
      ...
      <div
        className="pointer-events-none absolute inset-0"
        style={{
          background: "radial-gradient(ellipse 80% 60% at 50% 20%, rgba(236,72,153,0.12) 0%, transparent 60%), radial-gradient(ellipse 50% 40% at 80% 80%, rgba(168,85,247,0.1) 0%, transparent 55%)",
        }}
      />
      ...
      <div className="relative z-10 w-full max-w-md flex flex-col gap-8">
        ...
        <div className="text-center">
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-muted-foreground text-sm leading-relaxed">
            {phase === "done"
              ? `Em segundos ${data.personName || "ela"} vai receber isso e nunca mais vai esquecer.`
              : "Exatamente como vai aparecer quando voce compartilhar."}
          </motion.p>
        </div>
          ...
          >
            ...
            {pct < 100 && (
              <p className="text-xs text-muted-foreground mt-2">
                {[
                  !data.personName && "nome",
                  !data.coverPhotoUrl && "foto de capa",
                  !data.song && "musica",
                  !data.relationshipStart && "data do relacionamento",
                  !data.message && "mensagem",
                  data.galleryPhotoUrls.length === 0 && "galeria",
                ]
                  .filter(Boolean)
                  .slice(0, 2)
                  .join(" e ")}{pct < 100 ? " ainda pendentes" : ""}
              </p>
            )}
          </motion.div>
        )}
        ...
        <AnimatePresence>
          {phase === "done" && slug && (
            <SuccessState slug={slug} personName={data.personName} />
          )}
        </AnimatePresence>
        ...
        {phase === "idle" && (
          <div className="flex flex-col gap-3">
            <motion.button
              onClick={handlePublish}
              whileHover={{ scale: 1.03, boxShadow: `0 0 50px rgba(236,72,153,0.6), 0 0 100px rgba(168,85,247,0.3)` }}
              whileTap={{ scale: 0.97 }}
              className="w-full py-5 rounded-2xl text-base font-bold text-white cursor-pointer"
